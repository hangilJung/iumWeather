...
schedule.scheduleJob('17 46 * * * *', async () => {
    ...
    try {
      ...
      &nx=70&ny=70&dataType=JSON`
        ...
        const getData = await weatherOpenApiRequestDataResult.data.response.body.items.item
        ...
        const apiServerPassDataResult = await axios.post('http://localhost:3100/weather', {
            apiServerPassData,
        })
...
schedule.scheduleJob('00 00 * * * * ', async () => {
    ...
    try {
        ...
        for (let columnName of columnNames) {
            for (let row of getData) {
                if (row.category === columnName.key && row.fcstTime === nowTime + '00') {
        ...
        const apiServerPassDataResult = await axios.post('http://localhost:3100/weather/short', {
            apiServerPassData,
        })
...
schedule.scheduleJob('11 0 2 * * *', async () => {
    ...
    try {
        ...
        const apiServerPassDataResult = await axios.post('http://localhost:3100/weather/daily/temp', {
            apiServerPassData,
        })
